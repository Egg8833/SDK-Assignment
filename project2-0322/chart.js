...
function chart1(number){
  ...
  function renderData(){
    ...
    if(number <4){
      bigData.forEach((item)=>{
        let numb =0
        str.push(item.TimeStamp)
        int =item[newName].split('%')
        numb =int[0]*(1/100);  
        numb =numb.toFixed(4)
        num.push(numb)
  ...
  let chartNum =objName[number-1].num
  const myChart = new Chart(document.getElementById(`myChart${chartNum}`), config);
